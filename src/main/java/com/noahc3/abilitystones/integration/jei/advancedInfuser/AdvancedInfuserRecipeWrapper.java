...
public class AdvancedInfuserRecipeWrapper extends BlankRecipeWrapper{
    ...
    public AdvancedInfuserRecipeWrapper(AdvRecipe recipe) {
        ...
        for (int i=0; i < recipe.recipe.size(); i++) {
            ...
            switch (i) {
                case 0: st = recipe.recipe.get(0).item; st.setCount(recipe.recipe.get(0).count); ial.set(0, st); break;
                case 1: st = recipe.recipe.get(1).item; st.setCount(recipe.recipe.get(1).count); ial.set(1, st); break;
                case 2: st = recipe.recipe.get(2).item; st.setCount(recipe.recipe.get(2).count); ial.set(3, st); break;
                case 3: st = recipe.recipe.get(3).item; st.setCount(recipe.recipe.get(3).count); ial.set(4, st); break;
                default: break;
            }
        }

        ItemStack dust = new ItemStack(ModItems.magicalDust);
        dust.setCount(recipe.dustCost);
